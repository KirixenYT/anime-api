...
//     $('div.last_episodes.loaddub ul.items li').each((i, el) => {
//       list.push({
//         animeName: $(el).find('p.name > a').text(),
//         episodeNumber: $(el).find('p.episode').text().replace('Episode ', ''),
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const type = $('div.anime_info_body_bg > p.type > a').text();
